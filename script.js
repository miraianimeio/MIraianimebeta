...
function generateCardHtml(anime) {
    const mainTitle = anime.title.english || anime.title.romaji || "Untitled anime";
    const safeTitle = escapeSearchText(mainTitle);
    const posterUrl = escapeSearchText(anime.coverImage?.large || "logo.png");
...
if (anime.nextAiringEpisode) {
    ...
    return `
    <div class="anime-card" data-anime-id="${anime.id}" data-anime-title="${safeTitle}" data-anime-cover="${posterUrl}" data-anime-format="${animeFormat}" data-anime-episodes="${totalEpisodes}">
  <div class="poster-container">
    <a href="anime-details.html?id=${anime.id}" class="poster-link">
            <img src="${posterUrl}" alt="${safeTitle}" class="poster-image" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='logo.png';">
    </a>
        <span class="format-badge">${escapeSearchText(animeFormat)}</span>
        <button type="button" class="card-list-toggle" aria-label="Add ${safeTitle} to My List" title="Add to My List"><i class="fas fa-plus"></i></button>
...
document.addEventListener("click", event => {
    const listButton = event.target.closest(".card-list-toggle");
    if (!listButton || !window.MiraiList) return;
    event.preventDefault();
    event.stopPropagation();

    const card = listButton.closest(".anime-card");
    const animeId = Number(card?.dataset.animeId);
    if (!animeId) return;
    if (!localStorage.getItem("mirai_current_user")) {
        window.location.href = "index.html";
        return;
    }

    if (window.MiraiList.isInList(animeId)) {
        window.MiraiList.removeFromList(animeId);
    } else {
        window.MiraiList.addToList({
            id: animeId,
            title: card.dataset.animeTitle,
            cover: card.dataset.animeCover,
            format: card.dataset.animeFormat,
            episodes: card.dataset.animeEpisodes
        });
    }
    const isInList = window.MiraiList.isInList(animeId);
    listButton.classList.toggle("is-saved", isInList);
    listButton.title = isInList ? "Remove from My List" : "Add to My List";
    listButton.setAttribute("aria-label", `${isInList ? "Remove" : "Add"} ${card.dataset.animeTitle} ${isInList ? "from" : "to"} My List`);
    listButton.innerHTML = `<i class="fas fa-${isInList ? "check" : "plus"}"></i>`;
});
...
if (recommendedTrack && recommendedViewport) {
    ...
    let suppressRecommendedClick = false;
    ...
    document.addEventListener("pointermove", moveRecommendedDrag, { passive: false });
    ...
    const stopRecommendedDrag = (event) => {
        ...
        if (draggedRecommended) {
            suppressRecommendedClick = true;
            window.setTimeout(() => { suppressRecommendedClick = false; }, 0);
        }
    };
    ...
    document.addEventListener("pointerup", stopRecommendedDrag);
    document.addEventListener("pointercancel", stopRecommendedDrag);
    recommendedViewport.addEventListener("click", (event) => {
        if (!suppressRecommendedClick) return;
        event.preventDefault();
        event.stopPropagation();
        suppressRecommendedClick = false;
    }, true);
    ...
    const runRecommendedAutoScroll = () => {
        if (recommendedIsVisible && !isDraggingRecommended && !prefersReducedMotion.matches) {
            const loopWidth = recommendedTrack.scrollWidth / 2;
            if (loopWidth > 0) {
                recommendedViewport.scrollLeft += 0.55;
                if (recommendedViewport.scrollLeft >= loopWidth) {
                    recommendedViewport.scrollLeft = 0;
                }
            }
        }
        autoScrollFrame = requestAnimationFrame(runRecommendedAutoScroll);
    };
...
function injectSafeTutorialModal() {
    if (localStorage.getItem("mirai_seen_guide_v4")) return;
    ...
    cssStyle.textContent = `
        ...
        .m-modal-card {
            ...
            padding: 26px; border-radius: 12px; max-width: 500px; width: 100%;
        ...
        .m-step { display: flex; align-items: flex-start; gap: 15px; margin-bottom: 16px; }
    ...
    overlay.innerHTML = `
        <div class="m-modal-card">
            ...
            <p class="m-subtitle">Everything you need to find an anime, start an episode, and keep watching.</p>
            <div class="m-step">
                <div class="m-icon"><i class="fas fa-search"></i></div>
                <div class="m-text"><strong>Find something to watch</strong>Search by title, or use Genre, Types, and All Anime to browse.</div>
            </div>
            <div class="m-step">
                <div class="m-icon"><i class="fas fa-sliders"></i></div>
                <div class="m-text"><strong>Filter and sort</strong>On All Anime, narrow results by year, season, genre, type, or status.</div>
            </div>
            <div class="m-step">
                <div class="m-icon"><i class="fas fa-play"></i></div>
                <div class="m-text"><strong>Start watching</strong>Open a title, press Play Now, then choose an episode from the episode list.</div>
            </div>
            <div class="m-step">
                <div class="m-icon"><i class="fas fa-server"></i></div>
                <div class="m-text"><strong>Switch servers when needed</strong>Use the Sub or Dub server buttons. Try Next Server moves through the available choices.</div>
            </div>
            <div class="m-step">
                <div class="m-icon"><i class="fas fa-bookmark"></i></div>
                <div class="m-text"><strong>Keep your place</strong>Auto Next continues episodes, History remembers progress, and My List saves favorites.</div>
            </div>
    ...
    const closeGuide = () => {
        ...
        setTimeout(() => {
            overlay.remove();
            localStorage.setItem("mirai_seen_guide_v4", "true");
        }, 400);
    };
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (allAnimeContainer) {
        ...
        let requestedStatus = "";
        let requestedSeason = "";
        let requestedSort = "POPULARITY_DESC";
        ...
        async function loadAllAnimePaginated(page, direction = null) {
            ...
            const query = `
            query ($page: Int, $genre: String, $format: MediaFormat, $search: String, $seasonYear: Int, $status: MediaStatus, $season: MediaSeason, $sort: [MediaSort]) {
                Page (page: $page, perPage: 18) {
                    media (type: ANIME, sort: $sort, genre: $genre, format: $format, search: $search, seasonYear: $seasonYear, status: $status, season: $season) {
                        id
                        title { english romaji }
                        coverImage { large }
                        episodes
            ...
            const variables = { page: page, sort: requestedSort };
        ...
        const applyFilters = () => {
            ...
            requestedSort = document.getElementById("filter-sort").value;
            requestedSearch = "";
            currentPage = 1;
        ...
        document.getElementById("reset-filters").addEventListener("click", () => {
            ["filter-year", "filter-season", "filter-genre", "filter-type", "filter-status"].forEach(id => {
                document.getElementById(id).value = "";
            });
            document.getElementById("filter-sort").value = "POPULARITY_DESC";
            requestedYear = requestedGenre = requestedType = requestedStatus = requestedSeason = requestedSearch = "";
            requestedSort = "POPULARITY_DESC";
            currentPage = 1;
            loadAllAnimePaginated(currentPage);
        });
...
async function loadAiringSchedule(listId = "airingScheduleList") {
    ...
    const renderSchedule = (json) => {
        ...
        listEl.innerHTML = results.slice(0, 20).map(anime => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    style.textContent = `
        .anime-modal-overlay {
            ...
            z-index: 9999999; display: flex; align-items: flex-end; justify-content: center;
            opacity: 1; pointer-events: none; visibility: hidden;
            transition: visibility 0s linear 0.4s;
            font-family: 'Poppins', system-ui, -apple-system, sans-serif; padding: 0; box-sizing: border-box;
        }
        .anime-modal-overlay.active { pointer-events: auto; visibility: visible; transition-delay: 0s; }
        
        .anime-modal-card {
            background: #0d151c; border: 1px solid #1a2936; color: #fff;
            border-radius: 16px 16px 0 0; max-width: 860px; width: 100%; max-height: 88vh;
            overflow-y: auto; box-shadow: 0 -10px 50px rgba(0,0,0,0.85);
        ...
        .modal-spinner {
            width: 48px;
            height: 48px;
            border: 4px solid rgba(78, 217, 255, 0.15);
            border-top-color: #4ed9ff;
            border-radius: 50%;
        ...
        .anime-modal-btn-list { background: rgba(78, 217, 255, 0.1); color: #4ed9ff; border: 1px solid rgba(78, 217, 255, 0.2); }
        .anime-modal-btn-list:hover { background: rgba(78, 217, 255, 0.15); }
        .anime-modal-btn-list.in-list { background: rgba(34, 197, 94, 0.1); color: #22c55e; border-color: rgba(34, 197, 94, 0.2); }
            ...
            <div class="anime-modal-body">
                ...
                <div class="anime-modal-details">
                    ...
                    <div class="anime-modal-actions">
                        ...
                        <button class="anime-modal-btn anime-modal-btn-list" id="modal-list-btn" type="button">
                            <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg> <span id="modal-list-text">Add to List</span>
                        </button>
                    </div>
                </div>
            </div>
    ...
    const listText = document.getElementById("modal-list-text");
    let currentModalAnimeId = null;

    function updateModalListButton() {
        const user = localStorage.getItem("mirai_current_user");
        if (!user) {
            listBtn.style.display = "none";
            return;
        }
        listBtn.style.display = "inline-flex";
        const list = window.MiraiList?.getUserList() || [];
        const isInList = list.some(item => item.id === parseInt(currentModalAnimeId));
        if (isInList) {
            listText.textContent = "In My List";
            listBtn.classList.add("in-list");
        } else {
            listText.textContent = "Add to List";
    ...
    listBtn.addEventListener("click", () => {
        ...
        const isInList = list.some(item => item.id === parseInt(currentModalAnimeId));
        if (isInList) {
            window.MiraiList.removeFromList(parseInt(currentModalAnimeId));
        } else {
            const title = document.getElementById("modal-title").textContent;
            const cover = document.getElementById("modal-poster").src;
            const format = document.getElementById("modal-format").textContent;
            const episodes = document.getElementById("modal-episodes").textContent;
            window.MiraiList.addToList({
                id: parseInt(currentModalAnimeId),
                title: title,
                cover: cover,
                format: format,
                episodes: parseInt(episodes) || null
            });
        }
        updateModalListButton();
    });
