...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const uploadInput = document.getElementById("history-upload-input");
      ...
      if (e.target.files.length > 0) {
        ...
        reader.onload = function(ev) {
          try {
            ...
            const importedList = data.watchHistory || data.animeList || [];

            if (!Array.isArray(importedList) || importedList.length === 0) {
              alert('No history found in this file.');
              return;
            }
            ...
            importedList.forEach(entry => {
              ...
              if (id && !existingIds.has(parseInt(id))) {
                currentHistory.push({
                  ...
                  episode: entry.lastEpisodeWatched || 1,
                  totalEpisodes: entry.totalEpisodes || entry.episodes || null,
                  updatedAt: Date.now()
                });
            ...
            if (addedCount > 0) {
              alert(`Successfully imported ${addedCount} entries to your history!`);
            } else {
              alert('All entries in this file are already in your history.');
            }
          } catch (err) {
            alert('Invalid file. Please upload a valid Mirai history file.');
          }
        };
